...
echo "==> Step 4: Hammerspoon SpoonInstall"
# ~/.hammerspoon will become a symlink into this repo; Spoons/ is gitignored.
# SpoonInstall itself has to exist before init.lua can loadSpoon it, and it
# auto-installs every other spoon from there.
if [ ! -d "$DIR/home/.hammerspoon/Spoons/SpoonInstall.spoon" ]; then
  echo "    fetching SpoonInstall.spoon"
  mkdir -p "$DIR/home/.hammerspoon/Spoons"
  tmpzip="$(mktemp /tmp/SpoonInstall.XXXXXX.zip)"
  curl -sSfL https://github.com/Hammerspoon/Spoons/raw/master/Spoons/SpoonInstall.spoon.zip -o "$tmpzip"
  unzip -q -o "$tmpzip" -d "$DIR/home/.hammerspoon/Spoons/"
  rm -f "$tmpzip"
else
  echo "    SpoonInstall already present, skipping"
fi
...
# PATH here. Resolve the absolute path first and invoke that instead.
